...
HINSTANCE loadRealDLL()
{
    ...
    {
        strcpy_s(winmm_path, MAX_PATH, "winmm.Chained.dll");
    }
    else
    {
        GetSystemDirectoryA(winmm_path, MAX_PATH);
        strncat_s(winmm_path, MAX_PATH, "\\winmm.dll", _TRUNCATE);
    }
...
MCIERROR WINAPI fake_mciSendStringA(LPCSTR a0, LPSTR a1, UINT a2, HWND a3)
{
    static MCIERROR(WINAPI * funcp)(LPCSTR, LPSTR, UINT, HWND) = NULL;
    if (funcp == NULL)
...
MMRESULT WINAPI fake_auxGetDevCapsA(UINT_PTR a0, LPAUXCAPSA a1, UINT a2)
{
    static MMRESULT(WINAPI * funcp)(UINT_PTR, LPAUXCAPSA, UINT) = NULL;
    if (funcp == NULL)
...
{
    static MMRESULT(WINAPI * funcp)(UINT, LPDWORD) = NULL;
    if (funcp == NULL)
